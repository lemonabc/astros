...
class Asset {
    ...
    read() {
        ...
        if (nodeFs.existsSync(filepath)) {
            var fileData = nodeFs.readFileSync(filepath, 'utf8');
            fileData = this.modType == 'static' || this.fileType == 'img' ? fileData : fileData.toString();
            return fileData;
        }
        (astro.debug || this.debug) && console.warn('asset.read: ' + filepath + ' is miss')
        return null;
    }
    getContent(callback) {
        ...
        function next(asset) {
            try {
                ...
                if((astro.debug || self.debug) && hi >= 1 && (!asset.data || asset.data == 'undefined')){
