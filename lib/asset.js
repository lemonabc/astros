...
class Asset {
    ...
    get filePath() {
        ...
        let cfg = this.prjCfg;
        let filePath = '';
        let ext = this.fileType == 'img'?'.'+this.extname:('.' + (cfg[this.fileType + 'Ext'] || this.fileType));
        try {
            switch (this.modType) {
                case 'page':
                case 'webCom':
                case 'jsCom':
                    // 支持不带目录的形式，如：
                    // jslib/dialog.js
                    if(nodeFs.existsSync(nodePath.join(cfg[this.modType], this.name + ext))){
                        filePath = this.name;
                    }else{
            ...
            console.error('filePath error', this.info);
    ...
    get relPath() {
        ...
        if(this.modType == 'static'){
            require('assert')(typeof this._path, 'string', 'this.path is undefined; ' + this.info);
    ...
    getContent(callback) {
        ...
        function next(asset) {
            try {
                ...
                console.error(mw.name, asset.info,  error.stack);
            ...
            let read = () => {
                ...
                if (!asset) {
                    try{
                        ...
                        console.error('error in asset.getContents',  eee.stack);
                    }
                    resolve(assets);
                    return;
                }
