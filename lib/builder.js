...
class Rel {
    ...
    build(callback) {
        ...
        function printTimeSpan(des) {
            ts.push(new Date);
            console.info(des, (ts[ts.length - 1] - ts[ts.length - 2]) / 1000, 's');
        };
        ...
        async.series([
            ...
            function(cb) {
                self.dealJsCom(function() {
                    printTimeSpan('js组件资源耗时：');
                    cb();
                });
            },
            function(cb) {
            ...
            function(err, values) {
                ...
                console.info('发布路径：%s', prjCfg.release);
                console.info('总耗时：', (ts[ts.length - 1] - ts[0]) / 1000, 's');
                callback(prjCfg);
            }
        ]);
    };
    ...
    dealPage(callback) {
        ...
        try {
            let project = self.project,
                ...
                files = filePlus.getAllFilesSync(prjCfg.page, null, ['.' + prjCfg.cssExt, '.' + prjCfg.jsExt]);
            async.eachSeries(files, function(file, next) {
                ...
                switch (nodePath.extname(file)) {
                    case '.' + prjCfg.htmlExt:
                        // 针对页面可能没有CSS和JS的情况
                        // 如果存在HTML，则尝试发布JS和CSS
                        asset.release(function(){
                            var a = asset.clone();
                            a.fileType = 'css';
                            a.release(next);
                        });
    ...
    dealJsCom(callback) {
        ...
        let prjCfg = astro.getProject(self.project);
        let files = [];
        if (nodeFs.existsSync(prjCfg.jsCom)) {
            files = filePlus.getAllFilesSync(prjCfg.jsCom, null, ['.' + prjCfg.htmlExt, '.' + prjCfg.cssExt]);
        }
        let asset = new astro.Asset({
        ...
        async.eachSeries(files, function(file, next) {
            ...
            if (asset.fileType == 'img') {
                asset.name = nodePath.relative(prjCfg.jsCom, file).replace(new RegExp('\\' + nodePath.sep + 'img(' + nodePath.sep + '.*?)$'),
                    function(a, b) {
                        return b
                    });
            } else {
                asset.name = nodePath.dirname(nodePath.relative(prjCfg.jsCom, file))
            }
            asset.release(next);
        }, callback);
    };
    ...
    dealWebCom(callback) {
        ...
        let prjCfg = astro.getProject(self.project);
        let files = [];
        if (nodeFs.existsSync(prjCfg.webCom)) {
            files = filePlus.getAllFilesSync(prjCfg.webCom, null, ['.' + prjCfg.htmlExt, '.' + prjCfg.cssExt]);
        }
        let asset = new astro.Asset({
            project: self.project,
            modType: 'webCom',
        ...
        async.eachSeries(files, function(file, next) {
            ...
            asset.fileType = nodePath.extname(file) == ('.' + prjCfg.jsExt) ? 'js' : 'img';
            if (asset.fileType == 'img') {
                asset.name = nodePath.relative(prjCfg.webCom, file).replace(new RegExp('\\' + nodePath.sep + 'img(' + nodePath.sep + '.*?)$'),
                    function(a, b) {
                        return b
                    });
            } else {
                asset.name = nodePath.dirname(nodePath.relative(prjCfg.webCom, file));
            }
            asset.release(next);
        }, callback);
    };
    ...
    dealAssets(callback) {
        ...
        try {
            ...
            var files = filePlus.getAllFilesSync(assetsPath, null, prjCfg.ignore || []);
