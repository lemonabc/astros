...
function injectMethod(ast) {
    ...
    ast.setProject = function(options) {
        ...
        let optPath = astro.evn === 'release'?'rel.middlewares':'middlewares';
        util.setObjValue(options, optPath, 
            util.getObjValue(options, optPath)||[]);

        let mws = util.getObjValue(options, optPath);
        let mwsObjs = [];
        ...
        util.setObjValue(options, astro.evn === 'release'?'rel.mws':'mws', mwsObjs);
